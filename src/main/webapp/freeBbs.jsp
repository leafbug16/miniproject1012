...
<body>
  ...
  <form action="freeBbs" method="get" onsubmit="return searchCheck(this)">
  	<!-- new -->
    ...
    <tbody>
      <c:forEach var="list" items="${boardLists }">
        <tr style="text-align: center; border-top: 1px solid lightgray;">
          <td>${list.getNum() }</td>
          <td>
  					<a href="view?num=${list.getNum() }&back=freeBbs">${list.getTitle() }
...
<%-- <c:if test="${pageNumber != 1 }"><a href="bbs?pageNumber=${pageNumber - 1 }">이전</a></c:if>
  ...
  <button type="button" onclick="location.href='write?back=freeBbs';">글쓰기</button>
  <c:if test="${ph.showPrev }">
  	<!-- new 쿼리스트링 &ano=1 추가, anoBbs로 변경 -->
    <a href="<c:url value='/freeBbs${ph.sc.getQueryString(ph.beginPage-1)}&free=1'/>">&laquo;</a>
  </c:if>
  <c:forEach var="i" begin="${ph.beginPage }" end="${ph.endPage }">
  	<!-- new 쿼리스트링 &ano=1 추가, anoBbs로 변경 -->
    <a class="${ph.sc.page==i? 'check':'' }" href="<c:url value='/freeBbs${ph.sc.getQueryString(i)}&free=1'/>">${i }</a>
  </c:forEach>
  <c:if test="${ph.showNext }">
    <a href="<c:url value='/freeBbs${ph.sc.getQueryString(ph.endPage+1)}&free=1'/>">&raquo;</a>
  </c:if>
